  ...
  <header class="fixed top-0 w-full z-50 px-4 py-4 md:py-6 pointer-events-none">
    <div class="max-w-6xl mx-auto flex items-center justify-between px-4 py-2 
      ...
      <nav class="hidden md:flex items-center bg-white/10 p-1 rounded-xl border border-white/5">
        <NuxtLink v-for="nav in navigation" :key="nav.path" :to="`#${nav.id}`"
      ...
      <div class="flex items-center gap-4">
        <button class="hidden md:flex px-5 py-2 rounded-xl cursor-pointer bg-color text-white  text-sm font-semibold
    ...
    <transition name="mobile-menu">
      <div v-if="mobileMenuOpen" class="md:hidden fixed inset-x-4 top-24 bg-black/90 border border-white/10 
        ...
        <NuxtLink v-for="(nav, index) in navigation" :key="nav.path" :to="nav.path" @click="mobileMenuOpen = false"
        ...
        <button class="w-full py-4 bg-color text-black font-bold rounded-2xl">
          Contact Me
        </button>
      </div>
    </transition>
